...
export class AppComponent implements OnInit, OnDestroy {
  ...
  btp = { LS: 'waiting for active state', ES: 'waiting for active state', PS: 'waiting for active state',
  ...
  printBoardingPass() : void {
    const assets = this.bpp.assets.split(/[\r\n]/g).filter(a => !!a.trim().length)
    this.cuss2?.boardingPassPrinter?.setupAndPrintRaw(assets, this.bpp.coupon);
  }

  printBagTag() : void {
    const assets = this.btp.assets.split(/[\r\n]/g).filter(a => !!a.trim().length)
    this.cuss2?.bagTagPrinter?.setupAndPrintRaw(assets, this.btp.coupon);
  }
